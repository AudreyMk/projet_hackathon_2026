...
Usage :
    python project_manager.py status       # Tableau de bord projet
    python project_manager.py tasks        # Liste toutes les tâches
    python project_manager.py done <id>    # Marquer une tâche terminée
    python project_manager.py next         # Prochaines tâches à faire
    python project_manager.py report       # Génère un rapport HTML
    python project_manager.py check        # Vérifie les fichiers du projet
"""
...
TASKS = {
    ...
    "T15": {
        ...
        "duree_h": 3.0,
        "dependances": ["T14"],
        "livrable": "src/visualization/dashboard.py — lancé avec 'make dashboard'",
...
def cmd_check(state: ProjectState):
    ...
    checks = {
        ...
        "src/visualization/dashboard.py":              ROOT / "src/visualization/dashboard.py",
    ...
    if missing_data and HAS_RICH:
        console.print("\n[yellow]⚠️ Le master dataset n'existe pas encore. Lancer : [bold]make all[/][/]")
...
def main():
    ...
    if cmd == "status":
        ...
        print("Usage : python project_manager.py [status|tasks|done <id>|next|check|report|reset]")
