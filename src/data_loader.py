...
Produit les DataFrames consommés par dashboard_real.py.
...
# ── Chemins ────────────────────────────────────────────────────────────────
# data_loader.py est dans src/ — la racine projet est un niveau au-dessus
...
def build_annual_features(df_monthly: pd.DataFrame) -> pd.DataFrame:
    ...
    # Seules les stations ayant AU MOINS 5 ans de données sur 1961-1990
    # contribuent au calcul d'anomalie. Cela évite le biais de composition
    # du réseau : une station sans reference propre (ex: avant_1960.csv)
    # ne peut pas produire une anomalie fiable car on ne sait pas si elle
    # est naturellement froide ou chaude.
    if "tmoy" in df_by_station.columns and "num_poste" in df_by_station.columns:
        ...
        # Nombre d annees de donnees par station dans la periode de reference
        ref_counts = ref_period.groupby("num_poste")["tmoy"].count()
        # Garder seulement les stations avec au moins 5 ans de donnees valides
        ...
        # NaN ref_station => station sans donnees 1961-1990 suffisantes => anomalie = NaN (exclue)
